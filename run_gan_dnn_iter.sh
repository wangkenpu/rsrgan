...
if [ $stage -le 2 ]; then
  ...
  CUDA_VISIBLE_DEVICES="0,1,2,3" TF_CPP_MIN_LOG_LEVEL=2 \
    python scripts/train_gan_dnn_iter.py \
      --data_dir=$train_dir \
      --tr_list_file=$tr_list \
      --cv_list_file=$cv_list \
      --g_type="dnn" \
      --save_dir=exp/0923_gan_dnn_iter \
      --batch_size=256 \
...
if [ $stage -le 3 ]; then
  ...
  TF_CPP_MIN_LOG_LEVEL=2 python scripts/train_gan_dnn_iter.py \
      --decode \
      --data_dir=$train_dir \
      --test_list_file=$test_list \
      --g_type="dnn" \
      --save_dir=exp/0921_gan_dnn_iters_check1 \
      --batch_norm=true \
      --input_dim=257 \
      --output_dim=40 \
      --left_context=5 \
