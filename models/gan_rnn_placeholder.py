...
class GAN_RNN(Model):
  ...
  def __init__(self, sess, args, devices,
               cross_validation=False, infer=False, name='GAN_RNN'):
    ...
    self.inputs = tf.placeholder(
        tf.float32,
        [self.batch_size * self.num_gpu, None,
         self.input_dim * (self.left_context + 1 + self.right_context)],
        name="inputs")
    self.labels = tf.placeholder(
        tf.float32,
        [self.batch_size * self.num_gpu, None, self.output_dim], name="labels")
    self.lengths = tf.placeholder(
        tf.float32,
        [self.batch_size * self.num_gpu], name="lengths")
    # Batch Normalization
    self.batch_norm = args.batch_norm
    ...
    if infer:
      self.g_outputs = self.generator(self.inputs, self.labels,
                                      self.lengths, reuse=False)
    else:
      self.build_model(self.inputs, self.labels, self.lengths)
        ...
        with tf.device("/%s" % device):
          with tf.name_scope("device_%s" % idx):
            with variables_on_gpu():
              single_inputs = inputs[self.batch_size*idx:self.batch_size*(idx+1)]
              single_labels = labels[self.batch_size*idx:self.batch_size*(idx+1)]
              single_lengths = lengths[self.batch_size*idx:self.batch_size*(idx+1)]
              self.build_model_single_gpu(idx, single_inputs,
                                          single_labels, single_lengths)
