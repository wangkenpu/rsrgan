...
def train_one_iteration(sess, coord, model, tr_num_batch, iteration):
...
def eval_one_iteration(sess, coord, model, cv_num_batch, iteration):
...
def main(_):
    if not FLAGS.decode:
        filename = ("batch_num_%s.txt" % FLAGS.batch_size)
        ...
        train_batch_per_iter = 15000 * (256 / FLAGS.batch_size)
        valdi_batch_per_iter = 2000 * (256 / FLAGS.batch_size)
        ...
        with tf.Session(config=config) as sess:
            ...
            try:
                ...
                for iteration in range(max_iters):
                    ...
                    d_learning_rate = exponential_decay(iteration+1,
                        FLAGS.num_gpu, min_iters, FLAGS.d_learning_rate)
                    disc_noise_std = exponential_decay(iteration+1,
                        FLAGS.num_gpu, min_iters, FLAGS.init_disc_noise_std,
                        multiply_jobs=False)
                    sess.run(tf.assign(
                        tr_model.g_learning_rate, g_learning_rate))
                    sess.run(tf.assign(
