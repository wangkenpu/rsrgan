...
from models.dnn_trainer_single_gpu import DNNTrainer
...
def eval_one_epoch(sess, coord, model, cv_num_batch):
    ...
    for batch in xrange(int(cv_num_batch/FLAGS.num_gpu)):
        if coord.should_stop():
            break
        # now G iterations
        g_mse_loss, g_l2_loss, g_loss = sess.run([model.g_mse_losses,
                                                  model.g_l2_losses,
                                                  model.g_losses])
        counter += FLAGS.num_gpu
