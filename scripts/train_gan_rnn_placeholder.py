...
from models.gan_rnn_placeholder import GAN_RNN
from utils.misc import *
from utils.ops import *


def run_batch(sess, ids, inputs, labels, lengths, queue):
  """Read data using sess.run()"""
  while True:
    try:
      if labels is not None:
        batch_ids, batch_inputs, batch_lables, batch_lengths = sess.run(
            [ids, inputs, labels, lengths])
        queue.put([batch_ids, batch_inputs,
                   batch_lables, batch_lengths])
      else:
        batch_ids, batch_inputs, batch_lengths = sess.run(
            [ids, inputs, lengths])
        queue.put([batch_ids, batch_inputs, batch_lengths])
    except tf.errors.OutOfRangeError:
      # tf.logging.error("Out of range error occured when run data.")
      break


def train_one_iteration(sess, model, tr_num_batch, iteration, train_queue):
  ...
  for batch in range(int(tr_num_batch/FLAGS.num_gpu)):
    _, queue_inputs, queue_labels, queue_lengths = train_queue.get()
    # print("{}/{} shape:{}".format(batch+1, tr_num_batch, queue_inputs.shape))
    if queue_inputs.shape[0] != FLAGS.batch_size * FLAGS.num_gpu:
        continue
    # D iterations
    for d_step in range(model.disc_updates):
      ...
      _d_opt, d_rl_losses, \
      d_fk_losses, d_losses = sess.run(
          [model.d_opt, model.d_rl_losses, model.d_fk_losses, model.d_losses],
          feed_dict = {model.inputs: queue_inputs,
    ...
    for g_step in range(model.gen_updates):
      ...
      g_losses = sess.run(
          [model.g_opt, model.g_adv_losses,
           model.g_mse_losses, model.g_l2_losses, model.g_losses],
          feed_dict = {model.inputs: queue_inputs,
                       model.labels: queue_labels,
                       model.lengths: queue_lengths})
...
def eval_one_iteration(sess, model, cv_num_batch, iteration, valdi_queue):
  ...
  for batch in range(int(cv_num_batch/FLAGS.num_gpu)):
    _, queue_inputs, queue_labels, queue_lengths = valdi_queue.get()
    # D iterations
    d_rl_losses, \
    d_fk_losses, d_losses = sess.run(
        [model.d_rl_losses, model.d_fk_losses, model.d_losses],
        feed_dict = {model.inputs: queue_inputs,
                     model.labels: queue_labels,
                     model.lengths: queue_lengths})
    d_rl_loss = np.mean(d_rl_losses)
    ...
    g_l2_losses, g_losses = sess.run(
        [model.g_adv_losses, model.g_mse_losses,
         model.g_l2_losses, model.g_losses],
        feed_dict = {model.inputs: queue_inputs,
                     model.labels: queue_labels,
                     model.lengths: queue_lengths})
    g_adv_loss = np.mean(g_adv_losses)
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
      ...
      with tf.name_scope('model'):
        model = GAN_RNN(sess, FLAGS, devices, cross_validation=True, infer=True)
      ...
      # Data reader
      test_batch_queue = Queue.Queue(16)
      test_batch_thread = threading.Thread(target=run_batch,
          args=(sess, test_ids, test_inputs, None,
                test_lengths, test_batch_queue))
      test_batch_thread.daemon = True
      test_batch_thread.start()

      start = datetime.datetime.now()
      for batch in range(num_batch):
        try:
          queue_id, queue_inputs, queue_lengths = test_batch_queue.get()
          activations = sess.run(
              fetches=model.g_outputs,
              feed_dict={model.inputs: queue_inputs,
                         model.lengths: queue_lengths})
          sequence = activations * cmvn['stddev_labels'] + \
                  cmvn['mean_labels']
          save_result = np.vstack(sequence)
          writer.write_next_utt(write_ark_path, queue_id[0], save_result)
          print("[{}/{}] Write inferred {} to {}".format(
              batch+1, num_batch, queue_id[0], write_ark_path))
        except tf.errors.OutOfRangeError:
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
      ...
      with tf.name_scope('model'):
        ...
        print("=======================================================")
        tr_model = GAN_RNN(sess, FLAGS, devices, cross_validation=False)
        # tr_model and val_model should share variables
        print("=======================================================")
        print("|           Build Cross-Validation model              |")
        print("=======================================================")
        tf.get_variable_scope().reuse_variables()
        cv_model = GAN_RNN(sess, FLAGS, devices, cross_validation=True)
      ...
      train_batch_queue = Queue.Queue(32)
      valid_batch_queue = Queue.Queue(32)

      for iteration in range(max_iters):
        try:
          # Data reader
          train_batch_thread = threading.Thread(target=run_batch,
              args=(sess, tr_ids, tr_inputs, tr_labels,
                    tr_lengths, train_batch_queue))
          valdi_batch_thread = threading.Thread(target=run_batch,
              args=(sess, cv_ids, cv_inputs, cv_labels,
                    cv_lengths, valid_batch_queue))
          train_batch_thread.daemon = True
          valdi_batch_thread.daemon = True
          train_batch_thread.start()
          valdi_batch_thread.start()
