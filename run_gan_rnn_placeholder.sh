...
if [ $stage -le 2 ]; then
  ...
  python scripts/train_gan_rnn_placeholder.py \
      --data_dir=$train_dir \
      --tr_list_file=$tr_list \
      --cv_list_file=$cv_list \
      --g_type="res_lstm_l" \
      --save_dir=exp/0211_gan_res_lstm_l \
      --batch_size=8 \
      ...
      --num_gpu=2 || exit 1;
  CUDA_VISIBLE_DEVICES="0,1" TF_CPP_MIN_LOG_LEVEL=2 \
    python scripts/train_gan_rnn_placeholder.py \
      --data_dir=$train_dir \
      --tr_list_file=$tr_list \
      --cv_list_file=$cv_list \
...
if [ $stage -le 3 ]; then
  ...
  TF_CPP_MIN_LOG_LEVEL=2 python scripts/train_gan_rnn_placeholder.py \
      --decode \
      --data_dir=$train_dir \
      --test_list_file=$test_list \
      --g_type="res_lstm_l" \
      --save_dir=exp/0124_gan_res_lstm_l_1e-6 \
      --batch_norm=False \
      --input_dim=257 \
      --output_dim=40 \
      --left_context=0 \
